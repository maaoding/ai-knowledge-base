...
> **一句话结论：** 自回归 LLM 每次根据已有上下文计算下一个 Token 的 Logit，再按解码策略选出 Token 并重复；采样参数改变的是选择方式，不会给模型增加知识或推理能力。推理模型通常也以生成过程和测试时计算为基础，可能投入更多内部计算或 Token，但不等于一定展示完整的思维链。
...
- 系统指令（System Prompt：平台或开发者预设的固定指令，用户通常看不到，一般也不能修改）与聊天模板。
- 用户输入和历史对话。
- RAG 检索片段与工具结果。
- 已生成内容以及预留的后续输出空间。
...
**[KV Cache](/glossary#kv-cache)** 保存各层过去 Token 的 Key 和 Value，让下一步生成可以复用它们，而不必从头重复计算这些投影。它通常能明显加快逐 Token 解码，但代价是：
...
截至 2026 年 9 月，推理模型指 GPT-5 的内置思考模式、Claude 的扩展思考、Gemini 的思考模式，以及 DeepSeek-R1、Qwen 思考系列等开源推理模型：它们通常会在最终答案前进行额外的中间推理或计算；这些过程有时由思考 Token（Reasoning Tokens）承载，但不一定以可读的思维链（Chain-of-Thought）展示给用户。底层生成通常仍是自回归的：逐 Token 计算 Logit，并由模型与服务端的推理、解码策略推进生成。与普通模型相比，关键变化不只是“换一种采样”，还可能包括训练目标、推理流程和计算预算的设计。可以粗略理解为：普通模型更像直接答题，推理模型可能先打草稿再誊写，但草稿是否可见、如何产生，以具体模型和平台为准。

这类做法背后的思路叫测试时计算（Test-time Compute）：模型参数训练完成后通常固定不变，但可以在推理阶段投入更多计算，换取更好的结果。常见形式包括更长的思考预算（思考模式，thinking budget）、同一问题多次采样再挑选（Best-of-N）、以及用验证器筛选候选答案；并不是每种实现都会把这些过程展示成完整思维链。类比考试：草稿纸更多、时间更长，难题的正确率可能上升；但简单题这样做只是变慢，想得太久还可能把自己绕偏。

额外推理也会消耗计算资源，代价不只在最终答案：

- 如果额外推理以 Token 形式计入上下文，一段 500 Token 的思考加 100 Token 的答案，窗口里就可能要装下 600 Token；具体窗口如何计算，以模型和服务的说明为准。
- 额外推理会带来计算时间与用量。云服务可能把 reasoning tokens 计入输出、单独计价，或只展示摘要而隐藏细节；等待时间也取决于服务实现。因此“想得越深，回答越慢、越贵”是常见趋势，不是所有平台都完全相同的计费规则。
...
| 推理模型 / 测试时计算 | 通过额外推理、候选与验证投入计算，可能改善复杂任务表现 | 取决于模型与平台 | 可能增加延迟、窗口或用量占用与成本 |
...
- **“用上推理模型就不用管采样参数了”**：推理模型仍会进行逐步生成或其他额外推理，但是否能调 Temperature、Top-p，以及这些参数如何生效，取决于模型和平台；额外推理的窗口占用与计费也以官方说明为准。
...
因为它可能在最终答案前投入更多中间推理或候选验证计算，再给出结果。数学、代码这类可以分步展开并逐步核对的任务能从中获益；而简单问答本来一步就能答对，额外推理可能只是增加延迟、上下文或用量占用，想得太久还可能偏离正确方向。
