...
推理模型指 OpenAI 的 o1/o3 系列、DeepSeek-R1、Qwen 的思考系列这类模型：它们通常会在最终答案前进行额外的中间推理或计算；这些过程有时由思考 Token（Reasoning Tokens）承载，但不一定以可读的思维链（Chain-of-Thought）展示给用户。底层生成通常仍是自回归的：逐 Token 计算 Logit，并由模型与服务端的推理、解码策略推进生成。与普通模型相比，关键变化不只是“换一种采样”，还可能包括训练目标、推理流程和计算预算的设计。可以粗略理解为：普通模型更像直接答题，推理模型可能先打草稿再誊写，但草稿是否可见、如何产生，以具体模型和平台为准。
