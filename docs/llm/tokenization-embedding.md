...
- 用“高频合并”的直觉解释 BPE 这类子词算法为什么能学出子词。
- 区分 Token Embedding、位置表示和通用语义 Embedding。
- 区分静态嵌入与上下文嵌入，说明多义词为什么需要上下文信息。
...
前面列出的子词算法里，最常用的一类是 **BPE（Byte Pair Encoding，字节对编码）**。训练思路一句话：从单个字符出发，反复把语料中出现最多的相邻组合合并成一个新 Token，直到词表达到预设大小。

用一个只有两个词的微型语料走一遍：假设“ing”出现了 10 次，“ink”出现了 5 次。

1. 初始词表只有单个字符：`i`、`n`、`g`、`k`。统计相邻组合的出现次数：`i+n` 出现 15 次（两个词都以 in 开头），`n+g` 10 次，`n+k` 5 次。
2. 第一轮合并：出现最多的是 `i+n`，合并成新 Token `in`。词表变成 `in`、`g`、`k`；“ing”切成 `in + g`，“ink”切成 `in + k`。
3. 第二轮合并：`in+g` 出现 10 次，`in+k` 只有 5 次，于是 `in+g` 被合并成完整 Token `ing`。此后“ing”用一个 Token 表示，而“ink”停在 `in + k`——`in+k` 的出现次数不够高，没有获得下一轮合并，`k` 只能以零散字符的身份留在词表里。

这就是子词算法“常见组合整存、罕见组合现拼”的由来：高频组合一路合并成整词，低频组合只能用已有片段拼接。真实训练里，BPE 通常运行在字节或字符之上，词表大小训练前定死（常见几万到二十多万），合并次数用完即定型。
...
把 Embedding 放回历史里看更清楚。word2vec 这类早期方法学的是**静态嵌入（Static Embedding）**：每个词在训练结束后只有一个固定向量，与它出现在什么句子里无关。“打”在“打水”“打车”“打毛衣”三个短语里共享同一个向量，模型无法靠向量本身区分这三种“打”。

Transformer 的初始 Token Embedding 其实也是静态的——一张固定表格，每个 ID 对应一行，这正是上一节说“初始表示可以相同”的原因。真正的变化发生在后面的注意力层：同一个“打”进入不同句子后，各层不断结合周围的 Token 更新它的表示，最后每个位置得到**上下文嵌入（Contextual Embedding）**——“这是哪个 Token”被加工成了“它在这句话里是什么意思”。

多义词因此有了出路：在“打水”里，更新后的表示偏向取水的动作方向；在“打毛衣”里，偏向编织的方向。一句话概括这个改进：静态嵌入把词义钉死在词上，上下文嵌入把词义交给语境——这是 Transformer 相对旧方法的关键优势之一。
...
嵌入向量通常有几百到几千维，画不出来，但可以想象成高维空间里的点：意思相近的内容挨得近，无关的内容离得远。前文的余弦相似度，量测的就是这些点之间“方向是否对齐”。

几何里还值得记住“方向”。word2vec 时代有个著名观察：对几个词的向量做加减法，“国王 − 男 + 女”得到的方向大致指向“王后”——性别这一语义关系被表示成空间里一个稳定的位移方向，时态、单复数也有类似现象。要强调这是简化类比：真实结果并不总成立，现代上下文模型里这种“向量算术”更不直接，但它直观展示了嵌入空间的核心思想——语义关系可以被几何结构承载。

“距离近”之所以能被利用，是因为这个几何本来就是训练目标塑造的：训练通用 Embedding 模型时，常见做法（常称为**对比学习（Contrastive Learning）**）是让语义相近的文本对互相靠近、无关的对互相远离——几何结构是训练目标的产物，只在模型学过的语义范围内可靠。
...
在“没有固定换算比例”的大前提下，有一个对中文用户实用的总体倾向：表达同样的意思，中文的 Token 数通常比英文更多。机制上有三个原因：

- 英文有空格作天然切分线索，预切分容易；中文没有空格，切分主要靠子词算法统计字词组合，颗粒度更难控制。
- 词表构成跟着训练语料走。以英文为主的词表里，常见英文词往往整词入库，而常见中文词可能只拆到单字级别——“机器学习”可能被切成“机”“器”“学”“习”四个 Token。
- 字节级方案下，一个常见汉字在 UTF-8 里占 3 个字节，生僻字、符号和 emoji 还可能被拆成多个字节 Token；而多数英文字母一个字节就够。

新一代模型普遍加大了中文语料比例，常见中文词整词入库的情况越来越多，中文的“单价劣势”在缩小（截至 2026 年的趋势），但“同一段意思常占更多 Token”仍是许多模型的现状。实际含义与 [大模型入门](/llm/basics) 的成本原则相互印证：同样一份资料，中文版本更容易逼近上下文窗口上限，计费也更容易随之上涨。
...
- **“不同模型的 Embedding 向量可以互相比较”**：每个模型有自己的嵌入空间，维度和“坐标含义”都不同；A 模型算出的向量不能直接与 B 模型的向量算相似度，同一次检索里的文档与问题必须用同一个 Embedding 模型编码。
- **“向量的每一维都有人类可读的含义”**：单个维度通常不对应任何明确概念，可解释的是向量之间的关系——距离与方向。
...
### 4. 静态嵌入和上下文嵌入的区别是什么？为什么多义词需要后者？

<details>
<summary>查看答案</summary>

静态嵌入（如 word2vec）一词一向量，与语境无关：“打”在“打水”“打毛衣”里共享同一个向量，无法区分含义。Transformer 的初始 Token Embedding 也是静态查表，但经过注意力各层的上下文更新后，每个位置得到结合语境的表示，多义词在不同句子里被区分开。

</details>

### 5. 用 BPE 的合并思路解释：为什么“学习”在中文语料充足的模型里容易成为单个 Token，而生僻组合会被拆开？

<details>
<summary>查看答案</summary>

BPE 从单个字符出发，反复把语料中出现次数最多的相邻组合合并成新 Token。“学”“习”相邻出现得足够多，就会逐步被合并进词表，甚至直接合并成“学习”；生僻组合出现次数太少，没资格被合并，只能用已有的子词或字节 Token 现拼。词表大小有限，合并机会永远优先给高频组合。
...
- **应用方向：** [RAG 入门](/llm/rag-agent)展示语义 Embedding 在检索增强中的用途；向量怎么比相似、向量库怎么选，见 [检索与向量](/llm/retrieval)。
